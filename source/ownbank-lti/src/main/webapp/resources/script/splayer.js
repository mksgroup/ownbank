  ...
  function loadStream(url) {
     if(Hls.isSupported()) {
       ...
       hls = new Hls({debug:true, enableWorker : true, defaultAudioCodec : undefined});
	    ...
	    if (r) {
	      ...
	      for (var i=0, bufferLen=0; i<r.length; i++) {
	        if(pos >= r.start(i) && pos < r.end(i)) {
	          // play position is inside this buffer TimeRange, retrieve end
				// of buffer position and buffer length
	      ...
	      if(bufferLen <= 0.1 && v.paused === false && (pos-lastStartPosition) > 0.5) {
	        // don't create buffering event if we are at the end of the
			// playlist, don't report ended for live playlist
  ...
  function handleVideoEvent(evt) {
    ...
    switch(evt.type) {
       ...
       if(evt.target.duration - lastDuration <= 0.5) {
        // some browsers reports several duration change events with almost the
		// same value ... avoid spamming video events
        return;
       }
      ...
      case 'resize':
        ...
        break;
      case 'loadedmetadata':
        data = 'duration:' + evt.target.duration + '/videoWidth:' +
		 evt.target.videoWidth + '/videoHeight:' + evt.target.videoHeight;
       break;
      case 'loadeddata':
      case 'canplay':
      case 'canplaythrough':
      ...
      case 'error':
        ...
        break;
       case 'progress':
       data = 'currentTime:' + evt.target.currentTime + ',bufferRange:[' +
		 this.video.buffered.start(0) + ',' + this.video.buffered.end(0) +
		 ']';
       break;
      default:
      break;
